...
<div class="mx-auto max-w-5xl space-y-6">
  <!-- SEKCJA 1: 4 Kluczowe Wskaźniki KPI -->
  <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
    <!-- Seria dni -->
    <div
      class="app-card border-[var(--border-amber)] p-5 transition-all hover:border-[var(--brand-primary)]"
    >
      <div class="flex items-center justify-between">
        <span class="text-xs font-extrabold text-[var(--text-amber-brand)]"
          >Seria Nauki</span
        >
        <div class="rounded-xl bg-[var(--icon-bg-amber)] p-2">
          <Icon icon="ph:fire-bold" class="h-5 w-5 text-[var(--brand-primary)]" />
        </div>
      </div>
      <p
        class="mt-3 font-serif text-3xl font-bold text-[var(--text-serif-title)]"
      >
        {streakDays}
        {streakDays === 1 ? "dzień" : "dni"}
      </p>
      <p class="mt-1 text-xs font-bold text-[var(--text-muted)]">
        Utrzymuj codzienną dyscyplinę
      </p>
    </div>

    <!-- Łączne powtórki -->
    <div
      class="app-card p-5 transition-all hover:border-[var(--brand-primary)]"
    >
      <div class="flex items-center justify-between">
        <span class="text-xs font-extrabold text-[var(--text-primary)]"
          >Wykonane Powtórki</span
        >
        <div class="rounded-xl bg-[var(--icon-bg-stone)] p-2">
          <Icon icon="ph:arrows-clockwise-bold" class="h-5 w-5 text-[var(--brand-primary)]" />
        </div>
      </div>
      <p
        class="mt-3 font-serif text-3xl font-bold text-[var(--text-serif-title)]"
    ...
    <!-- Opanowane słowa -->
    <div
      class="app-card border-[var(--emerald-border)] p-5 transition-all hover:border-[var(--emerald-icon)]"
    >
      <div class="flex items-center justify-between">
        <span class="text-xs font-extrabold text-[var(--emerald-text)]"
          >Opanowane Słowa</span
        >
        <div class="rounded-xl bg-[var(--icon-bg-emerald)] p-2">
          <Icon icon="ph:check-circle-bold" class="h-5 w-5 text-[var(--emerald-icon)]" />
        </div>
      </div>
      <p
        class="mt-3 font-serif text-3xl font-bold text-[var(--text-serif-title)]"
    ...
    >
      <div class="flex items-center justify-between">
        <span class="text-xs font-extrabold text-[var(--text-amber-brand)]"
          >W Trakcie Nauki</span
        >
        <div class="rounded-xl bg-[var(--icon-bg-amber)] p-2">
          <Icon icon="ph:clock-bold" class="h-5 w-5 text-[var(--brand-primary)]" />
        </div>
      </div>
      <p
        class="mt-3 font-serif text-3xl font-bold text-[var(--text-serif-title)]"
  ...
  >
    <div
      class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[var(--border-default)] pb-3"
    >
      <div class="flex items-center gap-2">
        <Icon icon="ph:trend-up-bold" class="h-5 w-5 text-[var(--brand-primary)]" />
        <h3
          class="title-serif text-xl"
        >
          Aktywność (Ostatnie 7 dni)
        </h3>
      </div>
      <span class="text-xs font-bold text-[var(--text-muted)]">
        Łącznie w tym tygodniu: <strong
    ...
    <div class="pt-4 pb-2">
      <div
        class="grid grid-cols-7 gap-2 sm:gap-4 items-end h-40 border-b border-[var(--border-default)] pb-2"
      >
        {#each last7DaysData as day}
          ...
          >
            ...
            {#if day.count > 0}
              <span
                class="absolute -top-7 rounded-md bg-[var(--tooltip-bg)] px-2 py-0.5 text-[10px] font-bold text-[var(--tooltip-text)] shadow-md opacity-0 group-hover:opacity-100 transition-opacity"
              >
                {day.count} powt.
              </span>
            {/if}
            ...
            <!-- Słupek -->
            <div
              class="w-full max-w-[36px] rounded-t-lg transition-all duration-500 {day.isToday
            ...
            <!-- Etykieta dnia -->
            <span
              class="text-[11px] font-extrabold {day.isToday
  ...
  <div
    class="app-card p-6 space-y-5"
  >
    <div
      class="flex items-center gap-2 border-b border-[var(--border-default)] pb-3"
    >
      <Icon icon="ph:sparkle-bold" class="h-5 w-5 text-[var(--brand-primary)]" />
      <h3
        class="title-serif text-xl"
      >
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {#each categoryStats as cat}
        <div
          class="rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface-elevated)] p-4 space-y-2.5"
        >
          <div class="flex items-start justify-between gap-2">
            ...
            <span
              class="text-[10px] font-extrabold text-[var(--text-amber-brand)]"
  ...
  {#if hardestWords.length > 0}
    <div
      class="app-card border-[var(--border-amber)] p-6 space-y-4"
    >
      <div
        class="flex items-center justify-between border-b border-[var(--border-default)] pb-3"
      >
        <div class="flex items-center gap-2">
          <Icon icon="ph:warning-bold" class="h-5 w-5 text-[var(--text-amber-accent)]" />
          <h3
            class="title-serif text-xl"
          >
      ...
      <div class="space-y-2.5">
        {#each hardestWords as item}
          <div
            ...
            class="app-card-interactive group flex items-center justify-between bg-[var(--bg-surface-elevated)] p-3.5"
          >
            <div class="flex items-center gap-3">
              <span
                class="rounded-lg bg-[var(--icon-bg-amber)] p-2 text-[var(--brand-primary)]"
              >
                <Icon icon="ph:book-open-bold" class="h-4 w-4" />
              </span>
              <div>
                <h4
                  class="title-serif text-base group-hover:text-[var(--brand-primary)] transition-colors"
                >
                  {item.word.word}
                </h4>
                <p
                  class="text-xs font-bold text-[var(--text-muted)] line-clamp-1"
                >
            ...
            <div class="flex items-center gap-4 text-right">
              ...
              <span
                class="text-xs font-bold text-[var(--text-amber-accent)] group-hover:underline"
                >Podgląd</span
              >
            </div>
          </div>
  ...
  {#if activeWord}
    ...
    >
      <div
        ...
        class="modal-container max-w-xl p-6 space-y-5 cursor-default"
      >
        <div
          class="flex items-start justify-between border-b border-[var(--border-default)] pb-4"
        >
          <div>
            <h2
